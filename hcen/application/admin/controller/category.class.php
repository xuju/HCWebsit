...
class category extends common {
	...
	public function init() {
		...
		$tree->icon = array('&nbsp;&nbsp;&nbsp;│ ','&nbsp;&nbsp;&nbsp;├─ ','&nbsp;&nbsp;&nbsp;└─ ');
		$tree->nbsp = '&nbsp;&nbsp;&nbsp;';
		$data = $this->db->field('catid AS id,catname AS name,entitle as ename,parentid,`type`,modelid,listorder,member_publish,pclink,domain,display')->where(array('siteid'=>self::$siteid))->order('listorder ASC,catid ASC')->select();
		$array = array();
		foreach($data as $v) {
			if($v['type']=="0"){ 
		...
		$str  = "<tr class='text-c \$style'>
					...
					<td class='text-l'>\$parentoff\$spacer<a href='\$catlink' class='yzm_text_link'>\$ename</a></td>
					<td>\$cattype</td>
					<td>\$catmodel</td>
